...
export function PhaseGateExportButton() {
  ...
  const handleClick = async () => {
    ...
    try {
      const res = await fetch('/api/director/phasegate-excel', {
        method: 'POST',
        signal: ctrl.signal,
      })
      if (!res.ok) {
      ...
      a.href = url
      a.download = `Cornelis_CN6000_PhaseGate_StatusGrid_${dateSlug()}.xlsx`
  ...
  return (
    <div className="flex flex-col items-end gap-1">
      <button
        ...
        aria-label="Export phase-gate status grid as Excel"
      >
        <svg
          aria-hidden
          viewBox="0 0 16 16"
        ...
        {generating ? 'Generating…' : 'Export Status Grid'}
      </button>
      {state.kind === 'error' && (
